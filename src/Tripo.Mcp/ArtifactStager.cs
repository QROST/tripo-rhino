...
public sealed class ArtifactStager : IArtifactStager
{
    ...
    private static async Task WriteBundleAsync(
        ...
        CancellationToken cancellationToken)
    {
        Tripo.Bridge.BridgePaths.EnsurePrivateDirectory(bundleDirectory);
        ...
        await File.WriteAllTextAsync(manifestTemp, manifestJson, cancellationToken)
            .ConfigureAwait(false);
        Tripo.Bridge.BridgePaths.SetPrivateFileMode(manifestTemp);
        try
        {
    ...
    private static void PlaceEntry(string tempPath, string destinationPath)
    {
        ...
        {
            Tripo.Bridge.BridgePaths.TryDelete(tempPath);
        }
